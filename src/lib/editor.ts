...
export function buildGlassUniforms(
  ...
  return {
    ...
    u_originX: 0.5,
    u_originY: 0.5,
    u_glassOriginX: glass.centerX,
    u_glassOriginY: glass.centerY,
    u_worldWidth: 0,
    u_worldHeight: 0,
    u_scale: 1,
    u_offsetX: offsetX,
...
export function buildHalftoneUniforms(
  ...
  return {
    ...
    u_shadowRange: halftone.shadowEnabled ? halftone.shadowRange / 100 : 1.0,
    u_shadowInvert: halftone.shadowEnabled && halftone.shadowInvert ? 1 : 0,
...
export function buildGlitchyUniforms(
  ...
  return {
    ...
    u_glitchMode: ({ none: 0, smear: 3, channel: 4, posterize: 6, mirror: 7, burn: 8, hue: 9, scanhold: 10 } as const)[glitchy.glitchMode],
    u_glitchForm: ({ interlace: 0, wide: 1, mosaic: 2, compress: 3 } as const)[glitchy.glitchForm],
...
export function getGlowHPassConfig(state: EditorState, image: HTMLImageElement | HTMLVideoElement) {
  return {
    ...
    uniforms: {
      ...
      u_threshold: state.glow.threshold / 100,
      u_style: ['bloom', 'streaks'].indexOf(state.glow.style),
      u_opacity: 1.0,
    },
    speed: 0,
  };
