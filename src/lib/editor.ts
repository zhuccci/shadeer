...
export function buildHalftoneUniforms(
  ...
  return {
    ...
    u_offsetX: offsetX,
    u_offsetY: offsetY,
    u_dotScale: 52200 / (90 + 4.9 * halftone.scale),
    u_bw: halftone.blackAndWhite ? 1 : 0,
    u_pattern: ({ dots: 0, print: 1, lines: 2, cross: 3, dirty: 4, grunge: 5 } as const)[halftone.pattern],
    u_contrast: halftone.contrast / 50,
    u_brightness: (halftone.brightness - 50) / 50,
